...
class dark_light(nn.Module):
    def __init__(self, num_classes , length, both_flow):
        ...
        #self.features=nn.Sequential(*list(
        #    r2plus1d_34_32_ig65m(359, pretrained=True, progress=True).children())[:-2])
        self.self_attention = self_attention(self.hidden_size, 8 , hidden=self.hidden_size, n_layers=self.n_layers, attn_heads=self.attn_heads)
    ...
    def forward(self, x):
        if self.both_flow == 'True':
            ...
            output, maskSample = self.self_attention(x_cat,16)  # output(b,9,512),masksample(b,9)
